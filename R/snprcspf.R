## To install snprcspf package on LabKey server:
## In the install.package() function call immediately below, the version of
## the snprcspf package must match the version number within the most recent
## package. This can be found by examining the DESCRIPTION file within the
## package.
## Remove the leading three characters "## " from the four line command below
## and paste into the R console on the LabKey server.
##
## install.packages("d:Labkey data/snprcspf_0.0.302.tar.gz", type = "source",
##                  repos = NULL,
##                  lib = "c:/R Library")
##
## The snprcspf package has several dependencies and may not load if they
## cannot be found.
## For example, if the packages animalr, lubridate and readxl are missing, you should
## get a message similar to the following:
## ERROR: dependencies 'animalr', 'lubridate', 'readxl' are not available for package 'snprcspf'
## * removing 'c:/R Library/snprcspf'
## Warning messages:
##   1: running command '"C:/PROGRA~1/R/R-33~1.3/bin/x64/R" CMD INSTALL -l "c:\R Library" "d:Labkey data/snprcspf_0.0.302.tar.gz"' had status 1
## 2: In install.packages("d:Labkey data/snprcspf_0.0.302.tar.gz", type = "source",  :
## installation of package ‘d:Labkey data/snprcspf_0.0.302.tar.gz’ had non-zero exit status

## To correct this you will have to install the dependencies. One of these,
## animalr, is a locally produced package and is not on the CRAN mirrors. It
## is at "d:Labkey data/animalr_0.1.81.tar.gz" and is installed with the
## following command.
## install.packages("d:Labkey data/animalr_0.1.81.tar.gz", type = "source",
##                  repos = NULL,
##                  lib = "c:/R Library")
##
## The packages lubridate and readxl are on CRAN mirrors and can be installed
## with the following command after removing the three leading characters.
## install.packages(c("lubridate", "readxl"), type = "binary",
##   repos = "https://cran.revolutionanalytics.com",
##   lib = "c:/R Library")
##
## Sometimes a CRAN mirror cannot be reached or does not have the package
## needed. You can get a list of current mirrors at
## https://cran.r-project.org/mirrors.html
##
## If a package is there but is being reported as too old, you can simply
## install it as shown above or force an update of all packages.
...
##
##
## SPF Colony Testing Plate 96   06-09-16.xlsx has an extra low control
## "ICL CNT LOW (dup)".
## Controls with stri_detect_fixed(tolower(name), "(dup)") == TRUE are ignored.
